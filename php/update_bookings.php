...
<body>
    <?php
        ...
        while($resort = mysqli_fetch_row($resorts)) {
            ...
            $db_data = mysqli_query($con,"SELECT * FROM Booking_Log WHERE DATE(Checkout) = DATE_SUB(CURDATE(), INTERVAL 1 DAY);");
            ...
            $db_data = mysqli_query($con,"SELECT Room_Category, Checkin, Checkout, Room_Numbers FROM Booking_Log WHERE DATE(Checkin) = CURDATE();");
            while($tabledata=mysqli_fetch_row($db_data)) {
